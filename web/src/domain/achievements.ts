...
const DEFINITIONS: Definition[] = [
  ...
  { id: "tenTasks", name: "Dez tarefas", hint: "Dez tarefas concluídas.", target: 10, unit: "tarefas", repeatable: false, contributions: ({ own }) => own.map((completion) => step(completion)) },
  { id: "fiftyTasks", name: "Cinquenta tarefas", hint: "Cinquenta tarefas concluídas.", target: 50, unit: "tarefas", repeatable: false, contributions: ({ own }) => own.map((completion) => step(completion)) },
  { id: "hundredPoints", name: "Cem pontos", hint: "Cem pontos ganhos desde sempre.", target: 100, unit: "pontos", repeatable: false, contributions: ({ own }) => own.map((completion) => ({ ...step(completion), amount: completion.pointsAwarded })) },
  { id: "fiveHundredPoints", name: "Quinhentos pontos", hint: "Quinhentos pontos ganhos desde sempre.", target: 500, unit: "pontos", repeatable: false, contributions: ({ own }) => own.map((completion) => ({ ...step(completion), amount: completion.pointsAwarded })) },
  { id: "flawless", name: "Impecável", hint: "Recebeu uma nota 5 de alguém da casa.", target: 1, unit: "nota 5", repeatable: true, contributions: ({ own }) => own.filter((completion) => completion.rating === MAX_RATING).map((completion) => step(completion, reviewedAt(completion))) },
