...
/** A browser can hold a state written before crown titles existed. */
...
function withCrownTitles(state: StoredState): LocalState {
  return {
    ...state,
...
export class LocalStore {
  ...
  index(): HouseholdIndex {
    ...
    const state = withCrownTitles(this.takeLegacyState() ?? this.seed());
  ...
  read(inviteCode: string): LocalState | null {
    ...
    return raw === null ? null : withCrownTitles(JSON.parse(raw) as StoredState);
  }
