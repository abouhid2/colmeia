...
RSpec.describe Tasks::Complete do
  ...
  describe "a completion registered after the fact" do
    ...
    it "keeps the due date when the completion belongs to a cycle already closed" do
      task = household.tasks.create!(title: "Limpar banheiro", points: 20, recurrence: "weekly", due_on: Date.new(2026, 3, 20))
    ...
    it "sets a due date on a recurring task that never had one" do
      task = household.tasks.create!(title: "Regar as plantas", points: 5, recurrence: "daily")
    ...
    it "refuses a moment more than a year back" do
      task = household.tasks.create!(title: "Louça", points: 5)
